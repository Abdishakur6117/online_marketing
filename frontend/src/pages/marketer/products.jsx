...
function ProductForm({ editProduct, onSuccess, onCancel, showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    formData.images.forEach((file) => {
      data.append("images", file); // ✅ Tani waa sax – waxay la jaanqaadaysaa multer's `req.files`
    });
  ...
  const fetchProducts = async () => {
    ...
    try {
      const res = await axios.get("/api/products/my-products", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`, // ama meeshii token laga kaydiyey
        },
      });
      setProducts(res.data);
    } catch (error) {
      console.error("Fetch failed:", error);
      showToast("Error fetching your products", "error");
    } finally {
      setLoading(false);
    }
  };
