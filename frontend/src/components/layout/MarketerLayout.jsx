  ...
  return (
    <div>
      <div
        ...
        className={`flex justify-between items-center px-4 py-2 rounded cursor-pointer hover:bg-blue-100 ${
      ...
      {open && (
        <div className="ml-4 space-y-1">
          <NavItem to="/marketer/report/customers" label="Customer Report" />
          {/* <NavItem to="/admin/report/marketers" label="Marketer Report" /> */}
        </div>
      )}
    </div>
  );
...
export default function MarketerLayout() {
    ...
    return (
      <div className="min-h-screen grid place-items-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading marketer panel...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-[250px_1fr] bg-gray-100">
      ...
      >
        <h1 className="text-xl font-bold mb-6">Marketer Panel</h1>
        <NavItem to="/marketer" label="Dashboard" end />
        <NavItem to="/marketer/campaigns" label="My Campaigns" />
        <NavItem to="/marketer/products" label="Products" />
        <NavItem to="/marketer/productOrders" label="Product Orders" />
        <ReportDropdown />
      </aside>
      ...
      <div className="flex flex-col">
        ...
        <footer className="bg-white text-center py-2 text-sm border-t">
          &copy; {new Date().getFullYear()} Online Marketing Platform — Marketer
        </footer>
      </div>
    </div>
  );
...
function NavItem({ to, label, end = false }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `block px-4 py-2 rounded hover:bg-green-100 transition ${
          isActive
